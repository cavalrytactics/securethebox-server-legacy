...
class ModsecurityController():
    ...
    def modsecuritySetup(self):
        print("Checking if Service up with GET request")
        try:
            response = request.requests.get('http://nginx-modsecurity-charles.us-west1-a.securethebox.us')
            print(response.text)
        except:
            print("Cannot get response...")
        print("Setup WAF for:",self.serviceName,self.userName)
        ...
        while findPod:
            ...
            for i in pod_list:
                if f'nginx-modsecurity-{self.userName}' in str(i):
                    print("FOUND POD_ID:",str(i))
                    pod_id = str(i)
                    findPod=False
            counter+=1
            
        nginxGenerateConfig(clusterName,self.serviceName,self.userName)
        print("POD_ID:",pod_id)
        print("modsecuritySetup - Sleeping 10 seconds for nginx service to load...")
        time.sleep(10)
        try:
            print("Copying nginx file to Pod")
            subprocess.Popen([f"kubectl cp ./kubernetes-deployments/services/nginx-modsecurity/04_{self.clusterName}-{self.serviceName}-{self.userName}-nginx-2.conf "+pod_id+":/etc/nginx/nginx.conf"],shell=True).wait()
            subprocess.Popen([f"kubectl cp ./kubernetes-deployments/services/nginx-modsecurity/04_{self.clusterName}-{self.serviceName}-{self.userName}-modsecurity.conf "+pod_id+":/etc/nginx/modsec/modsecurity.conf"],shell=True).wait()
            ...
            try:
                ...
                nginxDeleteConfig(clusterName,"juice-shop",self.userName)
                print("COMPLETE!!!!")
            except:
                print("Error occured... on nginxReloadOut...")
                modsecuritySetup(self)    
