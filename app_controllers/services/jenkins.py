...
def jenkinsInstallPlugin(clusterName,userName):
    ...
    response = authed_session.post(url, data=payload, headers=headers)
...
def jenkinsGetSSHPublicKey(containerId):
...
def jenkinsGetSSHPrivateKey(containerId):
    private = check_output(["docker", "exec","-i",container_id,"bash","-c","cat id_rsa"])
    ssh_private_key = private.decode("utf-8")
    return ssh_private_key

def jenkinsCredentialsAddSSHPrivateKey(userName,clusterName,private_key):
    url1 = "http://jenkins-"+userName+"."+clusterName+".securethebox.us/credentials/store/system/domain/_/createCredentials"
    headers1 = {
        "Content-Type": "application/x-www-form-urlencoded"
    }
...
def jenkinsCreateJob(userName,clusterName):
    # Need to install plugins first
    # job title: deploy-to-kubernetes
    url1 = "http://jenkins-"+userName+"."+clusterName+".securethebox.us/view/all/createItem"
    headers1 = {
        "Content-Type": "application/x-www-form-urlencoded"
    }
