...
def gitlabCreateProject(clusterName, userName):
    ...
    subprocess.Popen([f"git push --set-upstream http://root:Changeme@gitlab-"+userName+"."+clusterName+".securethebox.us/root/juice-shop-"+userName+".git master"],shell=True).wait()
...
def gitlabPostResetPassword(token,session,clusterName,userName):
    ...
    page = authed_session.request("GET", url, headers=headers)
    ...
    submitform = authed_session.request("POST", submit_url, headers=headers, data=form_data)
...
def gitlabCreatePersonalAccessToken(userName, clusterName):
    ...
    response1 = authed_session.request("GET", url1)
    ...
    response2 = authed_session.request("POST", url2, data=payload, headers=headers, allow_redirects=True)
    ...
    response3 = authed_session.request("GET", url3, headers=headers3)
    ...
    response4 = authed_session.request("POST", url4, data=payload, headers=headers4, allow_redirects=True)
    ...
    response5 = authed_session.request("GET", url5, headers=headers5)
...
def gitlabMakeProjectPublic(clusterName,userName):
    ...
    response4 = authed_session.request("POST", url4, headers=headers4, data=payload4)
    print("Response code:",response4.status_code)

    # 2. enable outbound requests
    # http://gitlab-"+userName+"."+clusterName+".securethebox.us/admin/application_settings/network
...
def gitlabProjectAddDeployKey(jenkins_public_key,clusterName,userName):
    ...
    # 3
    url3 = "http://gitlab-"+userName+"."+clusterName+".securethebox.us/root/juice-shop-"+userName+"/settings/repository"
    headers3 = {
    'Host': "gitlab-"+userName+"."+clusterName+".securethebox.us",
    'Cookie': session_cookie2
    }
    ...
    payload4 = "utf8=%E2%9C%93&authenticity_token="+auth_token4+"&deploy_key%5Btitle%5D=jenkins-"+userName+"-root-public-key\
...
def gitlabProjectAddWebhook(clusterName,userName):
    ...
    response4 = authed_session.request("POST", url4, headers=headers4, data=payload4, allow_redirects=True)
    print("Response code:",response4.status_code)
